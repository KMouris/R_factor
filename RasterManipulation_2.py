    ...
    for k in range(0,nz):
        ...
        if np.array_equal(p_date, date_t) == True and np.array_equal(p_location, value[k,0:2] ) == True: #if the cell location AND date coincide
            ...
            if np.array_equal(p_date, date_t)== False: #if the dates are different, there is an error
                sys.exit("ERROR: The temperature file dates do nos coincide with the Precipitation raster dates. Please check user input")
            else:
...
path_precip=r'Y:\Abt1\hiwi\Oreamuno\GIS\03_R_Factor\Precipitation Files\Precip_Banja_corr_Text'  #Precipitation data
path_temp=r'Y:\Abt1\hiwi\Oreamuno\GIS\03_R_Factor\Temperature Files\Temperature_TextFiles'  #Temperature data
savepath= r'C:\Users\Oreamuno\Desktop\Banja Reservoir\Data\Raster_Manipulation_Results'  #Folder to save results (.CSV files for with all time data, per cell (station)
...
filenames_precip=glob.glob(path_precip + "\*.txt") #Get all txt files in the path directory, and save them in a list
if temp == True:
    CheckPath(path_temp)
    filenames_temp = glob.glob(path_temp + "\*.txt")  # Get all txt temperature files in the path directory, and save them in a list
